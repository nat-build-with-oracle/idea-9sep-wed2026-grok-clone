...
- Surface: main workspace, new chat/group picker, inspector, settings and marketplace overlays.
...
## Interaction states
Loading, empty searches, empty groups, sending, provider error/offline, routine paused/running/failed, upload validation, API failures, and persistence failures must be explicit. Public sample conversations are synthetic project-planning examples, not copied user history or live service output.

## Content voice
Short, clear, calm. Use Bot, group chat, computer, routine. No fabricated success claims.

## Implementation constraints
No third-party dependencies without explicit adoption. Proposed Xcode app plus Swift package core, Swift unit tests, and XCUITest. No localhost HTTP server is needed in the SwiftUI option. Never expose provider secrets in UI snapshots, logs, store exports, or fixtures.
