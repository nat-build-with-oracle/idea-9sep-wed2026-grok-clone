...
- **86 additional core tests**: 11 generation/coordinator, 15 Chat Completions SSE, 15 transport/request, 8 provider-setup/credential-routing, 12 model catalog, 9 Codex credential/request and 16 Codex SSE. Core total: **115**, using offline credentials/URLProtocol and actual temporary stores.
- **70 shell tests**: 38 original fixture/AppKit/persistence tests, 16 provider-presentation, 7 model-discovery and 9 Codex settings/flow tests. New Codex regressions cover bounded explicit import, session expiry, kind/root isolation, Stop/Retry with preserved partials/attribution, and failed-save credential cleanup. Total: **185 tests**.
...
- Swift build/typecheck, strict Swift-format lint, shell syntax checks, local ad-hoc code-signature verification and whitespace checks pass. The durable app has App Sandbox, outgoing network client and user-selected read-only file entitlements; the sample bundle has only App Sandbox. No broad-file-access or invented Keychain access-group entitlement is added.
