...
- **54 additional core tests**: 11 generation/coordinator, 15 SSE parser, 8 transport/request tests and 8 provider-setup/credential-routing tests and 12 model-catalog tests. Together with the repository tests, the core suite has **83 tests**. These use offline providers/credentials and URLProtocol, not live accounts.
- **38 shell tests**: 26 fixture/policy tests, 5 deterministic AppKit editor/lifecycle tests, 7 durable presentation-adapter tests. These include synthetic public-release fixtures, disabled-editor behavior during closing and a continuation-gated draft edit arriving while a prior save is suspended. The adapter tests use real temporary SQLite stores and one injected failing repository. The 16 provider-presentation tests add credential/metadata rollback, session-only sends/key expiration, storage-mode re-entry, attributed sends, group targeting, draft races, Stop/Retry and failed-shutdown recovery. Seven discovery-controller tests cover explicit lookup, sanitized failures, cancellation and stale results. Total: **144 tests** (83 core + 61 shell).
