...
- **38 shell tests**: 26 fixture/policy tests, 5 deterministic AppKit editor/lifecycle tests, 7 durable presentation-adapter tests. These include synthetic public-release fixtures, disabled-editor behavior during closing and a continuation-gated draft edit arriving while a prior save is suspended. The adapter tests use real temporary SQLite stores and one injected failing repository. The 13 provider-presentation tests add credential/metadata rollback, attributed sends, group targeting, draft races, Stop/Retry and failed-shutdown recovery. Total: **114 tests** (63 core + 51 shell).
